...
public partial class ResourceManager
{
    ...
    {
      var levelSaveData=  ProjectContext.Instance.SaveService.PlayerSaveData.AvailableLevels
            .Where(levelSaveData => levelSaveData.ID.Equals($"{groupID}_Level_Complete"))
            .LastOrDefault();

        Debug.Log(ProjectContext.Instance.SaveService.PlayerSaveData.AvailableLevels);

        if(levelSaveData!=null)
        {
            return levelSaveData.Levels;
        }
        else
        {
            return 0;
    ...
    public static void CompleteLevel(string groupId, int lvl)
    {
        ...
        levelsSaveData.ID = $"{groupId}_Level_Complete";
        levelsSaveData.Levels = lvl;
        ProjectContext.Instance.SaveService.PlayerSaveData.AvailableLevels.Add(levelsSaveData);
        ProjectContext.Instance.SaveService.Save();
